...
from mlProject.entity.config_entity import EntityRelationshipsConfig
...
class EntityRelationships:
    def __init__(self, config: EntityRelationshipsConfig):
        self.config = config
    ...
    def entityRelationships(self):
        ...
        for patient_id  in subjectids_unique:
            ...
            query = (
                ...
                "CREATE (n)-[:" + type_relation + "]->(e)"
            )
            try:
                self.query(query, parameters={"patient_id": patient_id})
            except Exception as e:
                logger.info("Issue when creating relations between entity and event nodes")
